...
double gamma2_correction(double v);
...
void raytrace_scene(std::vector<int>& pixel, synciterator& iter, scene_descriptor_type& scene, std::function<void(std::vector<int>::iterator, v3, scene_descriptor_type&)> trace_ray_functional)
{
    ...
    while ((xy = iter.next()) != std::make_pair(-1, -1))
    {
        if (xy.first == 0)
        {
            std::clog << "Line " << xy.second << std::endl<<std::flush;
        } 
        auto ray = scene.screen.get_corresponding_ray(xy.first, xy.second);
        ray = normalize(ray);

        int pixelindex = scene.screen.get_width() * xy.second + xy.first;
        pixelindex *= 3;
...
{
    std::vector<int> pixel(scene.screen.get_width() * scene.screen.get_height() * 3, 0);

    std::chrono::time_point<std::chrono::system_clock> start, end;
    start = std::chrono::system_clock::now();

    synciterator iter(scene.screen.get_width(), scene.screen.get_height());
    ...
    return std::make_tuple(pixel, scene.screen.get_width(), scene.screen.get_height());
}
